...
const CatalogIdPage = () => {
  ...
  React.useEffect(() => {
    const foundGood = data.find((good) => good.id == id);
    setGood(foundGood);
  }, [id]);
  ...
  return (
    ...
    <div style={{ borderBottom:"1px solid black"}} className="bg-white">
      ...
      <div className="w-full flex justify-between bg-white xl:flex-col">
        ...
        >
          {good.brand === "converse" ? <img style={{borderBottom: "1px solid black"}} className="xl:pr-24 xl:pl-24 p-6" src="https://upload.wikimedia.org/wikipedia/commons/3/30/Converse_logo.svg"/>: <img className="p-6" src="https://download.logo.wine/logo/Adidas/Adidas-Logo.wine.png"/>}
          ...
          <div style={{borderBottom: "1px solid black"}} className="p-6 pt-0">
          ...
          <div style={{borderBottom: "1px solid black"}} className="flex justify-between items-center gap-2 p-6">
            <span className="text-4xl font-bold">
              {good.price && good.price[0]}₽
            </span>
           <div className="flex gap-2">
           <Link href="/" className='hover:scale-95'><FaRegHeart size={30}/></Link>
            <Link href="/" className='hover:scale-95'><MdOutlineShoppingCart size={35}/></Link>
           </div>
          </div>

          <button style={{border:"1px solid black"}} className=" text-lg  p-3  m-6 bg-white text-zinc-900 rounded-xl hover:bg-black hover:text-zinc-50">
            купить в один клик
          </button>
        </div>
      </div>
